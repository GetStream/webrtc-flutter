...
- (void)getUserVideo:(NSDictionary*)constraints
  ...
  if (videoDevice) {
    ...
    // Stop any previous capturer before starting a new one. This used to be
    // macOS-only, which let a camera re-acquire race leave two AVCaptureSessions
    // running at once on iOS — double capture, double thermal load.
    ...
    if (selectedFormat == nil) {
      errorCallback(@"OverconstrainedError", @"No capture format available for the constraints");
      return;
    }
