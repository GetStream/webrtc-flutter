...
* [Android] Removed the per-frame thread hop from the simulcast encoder factory. `StreamEncoderWrapper` routed every `VideoEncoder` method through a dedicated single-thread executor and then immediately blocked on `future.get()`, so the calling encoder thread just waited — no parallelism was gained. With three simulcast layers that was three extra threads, three `Callable`+`FutureTask` allocations and six context switches per captured frame at 30 fps, plus the same round trip for the `getEncoderInfo()`/`getScalingSettings()` calls libwebrtc makes constantly. Calls now go straight through on the caller's thread. The frame-scaling fallback is unchanged.
* [Android] Fixed the simulcast fallback factory wrapping an already-wrapped factory, which nested one `StreamEncoderWrapper` inside another so every fallback encoder paid the wrapper twice per call.

### Android renderer

* [Android] Renderer surfaces are now opaque by default. Commit `05a6775` switched the root EGL context and every renderer surface to `CONFIG_RGBA`, which forces the system compositor to blend rather than overwrite — on every frame of every video tile. That was adopted to stop Impeller blending the background into the video texture on some devices, so it is a flag rather than a plain revert: pass `useAlphaEglConfig: true` in the `WebRTC.initialize` options (before the first renderer is created) to get the alpha config back.
* [Android] Added `RTCVideoRenderer.setViewSize`, which caps the Flutter texture at the size of the widget drawing it. Textures were sized to the incoming stream, so a 1080p remote in a small thumbnail allocated a full 1080p texture, paid a full-resolution `EglRenderer` draw every frame, and left the downscale to Flutter's compositor. The GL draw now does the downscale once. Never upscales.
* [Android] Added `RTCVideoRenderer.setFpsReduction`, so offscreen or backgrounded tiles can stop rendering. `setFpsReduction`/`pauseVideo` were implemented on the native renderer but had no callers and no method channel, so rendering could never actually be paused.
* [Android] The EGL surface is now only torn down and rebuilt when the *texture* size changes, rather than on every frame-resolution change. With textures capped at the widget size, a simulcast layer switch usually maps to the same texture size and no longer costs a synchronous cross-thread teardown on the frame-delivery thread.
* [Android] `setVideoTrack` no longer does a full `release()` + `init()` on every track assignment. That joined and respawned the render thread and destroyed and recreated the EGL surface each time; since the constructor already initializes, the first `srcObject` assignment always paid a redundant destroy/create, and grids that re-sort tracks churned render threads.

### Android capture & lifecycle

* [Android] `getUserMedia` no longer runs inline on the platform thread. Acquiring media does three camera enumerations, creates the capturer, starts capture and then waits up to **2000 ms** for the first frame, stalling every other method-channel call and the UI with it. It now runs on a dedicated single-threaded executor, which also serializes concurrent acquisitions instead of letting them race for the camera.
* [Android] Disabling a video track now stops the camera, not just the frames. `mediaStreamTrackSetEnabled(false)` only flipped the track flag, so the camera hardware, the capture GL thread and the local preview kept running at full rate while "muted" — with the platform capture indicator still lit.
* [Android] Capture now starts at the resolved format rather than the requested one. `startCapture` was passed the requested size while the resolved size was what got reported to Dart and what the background-resume path restarted with, so pre- and post-background capture formats could disagree. `videoSource.adaptOutputFormat` is now also applied, so the encoder never receives more than was asked for.
* [Android] Fixed the screen-share `SurfaceTextureHelper` leak. `getDisplayMedia` created a helper but never registered it, so `removeVideoCapturer` disposed neither the helper's GL thread and EGL surface **nor** the video source entry, which was nested inside the same check — one leaked thread per screen-share session.
* [Android] `OrientationAwareScreenCapturer` no longer allocates a `Configuration` and takes a monitor on every captured frame just to check device orientation. It listens for configuration changes instead.
* [Android] `LocalVideoTrack.onFrameCaptured` no longer takes a lock and allocates an iterator per captured frame for a processor list that is almost always empty.
...
* [iOS/macOS] A caller-initiated `setCaptureFormat` now re-bases the thermal throttle, so later step-downs scale from the new target rather than the resolution the camera was originally opened with. Deliberately not done for the throttle's own changes, which would make each step-down the new baseline and leave capture unable to recover as the device cools.
